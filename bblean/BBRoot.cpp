...
const TCHAR *cmds[] =
{
	...
	_T("-full"),        _T("-f"),
	_T("-center"),      _T("-c"),
	_T("-bitmap"),      _T("-hue"),     _T("-sat"),
	_T("tile"),         _T("center"),   _T("stretch"),

	_T("-solid"),
	_T("-gradient"),    _T("-from"),    _T("-to"),
	_T("-mod"),         _T("-fg"),      _T("-bg"),
...
#define WP_FULL 3
	...
	int next_token(void)
	{
		_tcslwr_s(NextToken(token, MAX_PATH, &cptr), MAX_PATH);
		return (int)get_string_index(token, cmds);
	}
	...
	int parse_command(void)
	{
		while (*cptr)
		{
			int s = next_token();
		cont_1:
			if (_T('-') != token[0] && 0==bmp) goto img_name;
			switch (s) {
			case E_bitmap:
				while(*cptr)
				{
			...
			case E_gradient:
				...
				{
					s = next_token();
					int t; int ParseType(TCHAR *);
					if (-1 != (t = ParseType(token)))
					{
						gradient = 1;
						type = t;
						if (_tcsstr(token, cmds[E_interlaced]))
							interlaced = true;
					}
					else
						if (s == E_from)
					...
					if (s == E_interlaced)
					{
						...
						if (s == E_fg)
						{
							...
							if (s == E_bg)
							{
								...
								if (*token>=_T('0') && *token <=_T('9') && mod<3)
								{
									mody = _ttoi(token);
									if (++mod==2) modx = mody;
								}
	...
	//===========================================================================
public:
	HBITMAP make_root_bmp(const TCHAR *command)
	{
		cptr = command;
...
//===========================================================================
HBITMAP read_bitmap(LPCTSTR path, bool delete_after)
{
...
//===========================================================================
HBITMAP load_desk_bitmap(LPCTSTR command)
{
	TCHAR token[MAX_PATH];
	const TCHAR *cptr = command;

	_tcslwr_s(NextToken(token, MAX_PATH, &cptr), MAX_PATH);

	if (NULL==_tcsstr(token, _T("bsetroot")) && NULL==_tcsstr(token, _T("bsetbg")))
	{
	...
	make_bb_path(exe_path, MAX_PATH, _T("bsetroot.exe"));

	unsigned long bsrt_vernum = getfileversion(exe_path, NULL, 0);
	...
	TCHAR bsetroot_bmp[MAX_PATH];
	make_bb_path(bsetroot_bmp, MAX_PATH, _T("$bsroot$.bmp"));

	TCHAR spawn_cmd[1024];
	_sntprintf_s(spawn_cmd, 1024, _TRUNCATE, _T("\"%s\" %s -save \"%s\""),
		exe_path, cptr, bsetroot_bmp);

	TCHAR bb_path[MAX_PATH];
	GetBlackboxPath(bb_path, MAX_PATH);

	//dbg_printf(_T("command <%s>"), spawn_cmd);
	if (false == ShellCommand(spawn_cmd, bb_path, true))
	{
		BBMessageBox(MB_OK, NLS2(_T("$BBError_rootCommand$"),
			_T("Error: Could not execute rootCommand:\n%s")), spawn_cmd);
		return NULL;
	}
