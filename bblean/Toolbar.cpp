...
const struct cfgmenu tb_main[] = {
	{ NLS0(_T("Placement")),      NULL, tb_sub_placement },
	{ NLS0(_T("Settings")),       NULL, tb_sub_settings },
	{ NLS0(_T("Always On Top")),  _T("@ToolbarAlwaysOnTop"), &Settings_toolbarOnTop },
	{ NLS0(_T("Auto Hide")),      _T("@ToolbarAutoHide"), &Settings_toolbarAutoHide },
	{ NLS0(_T("Edit Workspace Names")), _T("@BBCore.EditWorkspaceNames"), NULL },
	{ NULL }
};

const TCHAR *placement_strings[] =
{
	_T("TopLeft")       ,   //#define PLACEMENT_TOP_LEFT     0
	_T("TopCenter")     ,   //#define PLACEMENT_TOP_CENTER   1
	_T("TopRight")      ,   //#define PLACEMENT_TOP_RIGHT    2
	_T("BottomLeft")    ,   //#define PLACEMENT_BOTTOM_LEFT  3
	_T("BottomCenter")  ,   //#define PLACEMENT_BOTTOM_CENTER 4
	_T("BottomRight")   ,   //#define PLACEMENT_BOTTOM_RIGHT 5
	NULL
	};

const TCHAR Toolbar_ClassName[] = _T("BBToolbar"); // Window class etc.
...
void Toolbar_AutoHide(bool);
void Toolbar_set_pos(void);

bool Toolbar_ShowingExternalLabel;
TCHAR Toolbar_CurrentWindow[256];
TCHAR Toolbar_CurrentTime[80];
TCHAR Toolbar_WorkspaceName[80];
...
bool Toolbar_moving;
HWND Toolbar_hwnd;
bool Toolbar_force_button_pressed;
int tbClockW;
...
RECT *capture_rect;
bool *pressed_state;
void (*MouseButtonEvent)(unsigned message, LPARAM id);
LPARAM button_id;
...
void Toolbar_setclock(void)
{
	...
	struct tm ltp; localtime_s(&ltp,&systemTime);
	_tcsftime(Toolbar_CurrentTime, sizeof(Toolbar_CurrentTime)/sizeof(TCHAR), Settings_toolbarStrftimeFormat, &ltp);
	if (Toolbar_hidden)
	{
	...
	else
	{
		...
		bool seconds = _tcsstr(Settings_toolbarStrftimeFormat, _T("%S")) || _tcsstr(Settings_toolbarStrftimeFormat, _T("%#S"));
...
bool check_mouse(HWND hwnd)
{
	...
	return PtInRect(&rct, pt) != FALSE;
}

/*
void drop_style(HDROP hdrop)
{
    TCHAR filename[MAX_PATH]; filename[0]=0;
...
void Toolbar_AutoHide(bool hide)
{
