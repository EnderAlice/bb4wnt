...
// workspace names
struct string_node *deskNames;

// application listed in StickyWindows.ini
struct string_node * stickyNamesList;
...
struct toptask   * pTopTask;

//====================
// local functions

void WS_LoadStickyNamesList(void);
bool WS_IsSticky(HWND hwnd);
void WS_MakeItSticky(HWND hwnd);
void WS_ClearStickyness(HWND hwnd);

void DeskLeft(void);
void DeskRight(void);
void DeskSwitch(int);

void MoveWindowToWkspc(HWND, int);
void NextWindow(bool, int);

void switchToDesktop(int desk);
void setDesktop(HWND hwnd, int desk, bool switchto);
void MoveWindowToDirectWS(HWND hwnd, int ws_number, bool follow);

void EditWorkspaceNames(void);
void AddDesktop(int);
void SetNames(void);
...
int Workspaces_getDesktop(HWND h);
bool is_valid_task(HWND hwnd);
int FindTask(HWND hwnd);
void SetTopTask(struct tasklist *, int f);
HWND search_top_window(int scrn, HWND hwnd_exclude);
bool focus_top_window_beside(HWND hwnd_exclude);
void post_desk_refresh(void);
void post_task_refresh(void);
...
void EditWorkspaceNames(void)
{
	...
	if (IDOK == EditBox(
		_T("bbLean"),
		NLS2(_T("$BBWorkspace_EditNames$"), _T("Workspace Names:")),
		Settings_workspaceNames,
		Settings_workspaceNames,
		sizeof(Settings_workspaceNames)/sizeof(TCHAR)
		))
	{
...
{
	freeall(&deskNames);
	const TCHAR *names = Settings_workspaceNames; int i;
	for (i = 0; i < Settings_workspaces; ++i)
	{
		TCHAR wkspc_name[80];
		if (0 == *NextToken(wkspc_name, 80, &names, _T(",")))
			_sntprintf_s(wkspc_name, 80, _TRUNCATE, NLS2(_T("$BBWorkspace_DefaultName$"), _T("Workspace %d")), i+1);
...
HWND get_default_window(HWND hwnd)
{
	if (NULL == hwnd)
	{
		...
		if (NULL == hwnd || GetCurrentThreadId() == GetWindowThreadProcessId(hwnd, NULL))
		{
			...
			hwnd = GetLastActivePopup(GetAncestor(hwnd, GA_ROOTOWNER));
			SwitchToWindow(hwnd);
		}
	}
...
LRESULT send_syscommand(HWND hwnd, UINT SC_XXX)
{
...
void Workspaces_Command(UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case BB_LISTDESKTOPS:
		...
		//====================
	case BB_WORKSPACE:
		//dbg_printf(_T("BB_WORKSPACE %d %x"), wParam, hwnd);
		switch (wParam)
		{
			// ---------------------------------
		...
		case BBWS_MOVEWINDOWTOWS:
			...
			//                     dbg_printf(_T("BBWS_MOVEWINDOWTOWS hwnd=%x, lParam=%s"), hwnd, lParam);

			args = (const TCHAR *)lParam;
			NextToken(ws, 10, &args);
			ws_number = _ttoi(ws)-1;

			if (_tcslen(args)==0)
				follow = Settings_followMoved;
			else
				follow = !_tcsicmp(args, _T("true"));
	...
	case BB_BRINGTOFRONT:
		WS_BringToFront(hwnd, (wParam & BBBTF_CURRENT) == BBBTF_CURRENT);
		break;
	...
	default:
		{
			...
			case BB_WINDOWMINIMIZETOTRAY:
				tasklist *p;
				HWND hParent = (HWND)GetWindowLongPtr(hwnd,GWLP_HWNDPARENT); // 64bit
				dolist(p, taskList){
					if (p->hwnd == hwnd || p->hwnd == hParent){
						MinimizeToTray(hwnd, p->hwnd);
					}
				}
				break;
			}
...
//===========================================================================
const TCHAR BBSHADE_PROP[] = _T("BBNormalHeight");
...
{
	LRESULT shade = SendMessage(hwnd,
							RegisterWindowMessage(_T("BBLEANSKIN_MSG")),
							BBLS_GETSHADEHEIGHT, 0);

	if (shade) return (int)shade;
	...
	//dbg_printf(_T("caption %d  border %d"), caption, border);
...
void WS_SendStickyInfo(HWND hwnd)
{
...
bool WS_IsSticky(HWND hwnd)
{
	return (VWM_STICKY & vwm_get_status(hwnd)) == VWM_STICKY;
}
...
bool check_sticky_name(HWND hwnd)
{
	...
	if (NULL == stickyNamesList || 0 == GetAppByWindow(hwnd, appName, MAX_PATH))
		return false;

	//check appnames from _T("StickyWindows.ini")
	struct string_node * sl;
	dolist (sl, stickyNamesList)
		if (0==_tcsicmp(appName, sl->str))
		{
			//dbg_printf(_T("sticky: %s"), appName);
			return true;
		}
	return false;
}
...
{
	TCHAR path[MAX_PATH]; TCHAR buffer[256];
	freeall(&stickyNamesList);
	FindConfigFile(path, MAX_PATH, _T("StickyWindows.ini"), NULL);
	FILE* fp = FileOpen(path);
	if (fp)
	{
		while (ReadNextCommand(fp, buffer, 256))
...
void min_rest_helper (LPARAM cmd)
{
	list_node *p, *lp = NULL;
...
{
	HWND hwndLAP = GetLastActivePopup(GetAncestor(hwnd, GA_ROOTOWNER));
...
void get_desktop_info(DesktopInfo *deskInfo, int i)
{
	...
	if (sp) _tcsncpy_s(deskInfo->name, 32, sp->str, _TRUNCATE);
}
...
int Workspaces_getDesktop(HWND hwnd)
{
...
void MoveWindowToWkspc(HWND hwnd, int where)
{
	if (NULL == hwnd) return;
...
void MoveWindowToDirectWS(HWND hwnd, int ws_number, bool follow)
{
	...
	//vwm_set_workspace(hwnd, ws_number);

	if (follow)
	{
		//       dbg_printf(_T("BBWS_MOVEWINDOW followMoved!"));
		SwitchToWindow(hwnd);
	}
	else
	{
		//       dbg_printf(_T("BBWS_MOVEWINDOW not followMoved!"));
		post_task_refresh();
		post_desk_refresh();
...
void get_ico(struct tasklist *tl)
{
	if (HICON hIco = GetIconFromHWND(tl->hwnd)){
		if (tl->orig_icon != hIco)
		{
...
void get_text(struct tasklist *tl)
{
	GetWindowText(tl->hwnd, tl->caption, sizeof(tl->caption)/sizeof(TCHAR));
}
...
//===========================================================================
#if 0
void debug_tasks(WPARAM wParam, HWND hwnd)
{
	int n = wParam;
	int task = FindTask(hwnd);
	TCHAR buffer[256];
	GetWindowModuleFileName(hwnd, buffer, 256);
	const TCHAR *p = get_file(buffer);
	bool BBA = false;
	if (NULL == hwnd)
	{
	...
	static TCHAR *msg[8] = { _T("null"), _T("add"), _T("remove"), _T("activateshell"), _T("activate"), _T("minmax"), _T("redraw"), _T("taskman") };

	dbg_printf(_T("[msg %d %s] hwnd=%x task=%d app=%s BBActive=%d"), n, n<8 ? msg[n] : _T("xxx"), hwnd, task, p, BBA);
}
#define DEBUG_TASKS
#endif
...
{
	bool extra = (wParam & 0x8000) == 0x8000;
	...
	hshell_windowactivated:
		{
			...
			{
				tl->active = true;
				bool was_iconic = (VWM_ICONIC & vwm_get_status(hwnd)) == VWM_ICONIC;
				int n = vwm_get_desk(hwnd);
...
API_EXPORT(void) SetTaskWorkspace(HWND hwnd, int wkspc)
{
...
API_EXPORT(bool) GetTaskLocation(HWND hwnd, struct taskinfo *t)
{
...
API_EXPORT(bool) SetTaskLocation(HWND hwnd, struct taskinfo *t, UINT flags)
{
	int c = currentScreen;
