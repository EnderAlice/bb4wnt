...
{
	struct string_node *b; size_t l = _tcslen(s)+1;
	b = (struct string_node *)m_alloc(sizeof *b + sizeof(TCHAR)*l);
	_tcsncpy_s(b->str, l, s, _TRUNCATE);
	b->next = NULL;
	return b;
};
...
TCHAR *new_str(const TCHAR *s) {
	TCHAR *p;
	size_t l;
	if(s)
	{
		l = _tcslen(s) + 1;
		p = (TCHAR*)m_alloc(sizeof(TCHAR)*l);
		_tcsncpy_s(p,l,s,_TRUNCATE);
	}
	else
	{
		p = NULL;
