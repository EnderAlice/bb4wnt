...
unsigned char _sqrt_table[SQR_TAB_SIZE];

#define DITH_TABLE_SIZE (256+8)
int bits_per_pixel = -1;
unsigned char _dith_r_table[DITH_TABLE_SIZE];
unsigned char _dith_g_table[DITH_TABLE_SIZE];
unsigned char _dith_b_table[DITH_TABLE_SIZE];
unsigned char add_r[16], add_g[16], add_b[16];
...
class bImage
{
	...
	static void init_dither_tables(void)
	{
		...
		else
		{
			...
			{
				_dith_r_table[i] = (int)imin(255, i & -red_bits    );
				_dith_g_table[i] = (int)imin(255, i & -green_bits  );
				_dith_b_table[i] = (int)imin(255, i & -blue_bits   );
			}
...
extern "C" API_EXPORT(void) MakeGradient(HDC hDC, RECT rect, int type, COLORREF colour_from, COLORREF colour_to, bool interlaced, int bevelStyle, int bevelPosition, int bevelWidth, COLORREF borderColour, int borderWidth)
{
...
extern "C" API_EXPORT(void) MakeGradientEx(HDC hDC, RECT rect, int type, COLORREF colour_from, COLORREF colour_to, COLORREF colour_from_splitto, COLORREF colour_to_splitto, bool interlaced, int bevelStyle, int bevelPosition, int bevelWidth, COLORREF borderColour, int borderWidth)
{
	if (borderWidth)
	{
	...
	if (type >= 0)
	{
		...
		bool is_vertical   = (type == B_SPLITVERTICAL)   || (type == B_MIRRORVERTICAL);
		bool is_horizontal = (type == B_SPLITHORIZONTAL) || (type == B_MIRRORHORIZONTAL);
		bool is_split      = (type == B_SPLITVERTICAL)   || (type == B_SPLITHORIZONTAL);
		bool is_mirror     = (type == B_MIRRORVERTICAL)  || (type == B_MIRRORHORIZONTAL);
...
extern "C" API_EXPORT(void) CreateBorder(HDC hdc, RECT *prect, COLORREF borderColour, int borderWidth)
{
