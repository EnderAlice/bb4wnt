...
void * _m_alloc(unsigned n, const TCHAR *file, int line);
void * _c_alloc (unsigned n, const TCHAR *file, int line);
void   _m_free(void *v);
void * _m_realloc (void *v, unsigned s, const TCHAR *file, int line);
...
#define m_alloc(s)      _m_alloc(s,_T(__FILE__),__LINE__)
#define c_alloc(s)      _c_alloc(s,_T(__FILE__),__LINE__)
#define m_realloc(p,s)  _m_realloc(p,s,_T(__FILE__),__LINE__)
#define m_free(p)       _m_free(p)
#define new             (_m_setinfo(_T(__FILE__),__LINE__), false) ? NULL : new

#define m_alloc_check_leaks(title) \
    if (alloc_size) { \
        BBMessageBox(MB_OK, _T("#") title _T(" - Memory Check#Memory leak detected: %d bytes"), alloc_size); \
        m_alloc_dump_memory(); \
    }
...
#define c_alloc(n) memset(malloc(n),0,n)
