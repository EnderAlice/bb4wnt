...
#define BB_SETSTYLE             10104 /* lParam: const TCHAR* stylefile */
...
#define BB_SETTOOLBARLABEL      10304   /* Set the toolbar label to (LPCTSTR)lParam (returns to normal after 2 seconds) */
...
#define BB_WORKSPACE            10505
  ...
  #define BBWS_ADDTASK          25
  #define BBWS_REMOVETASK       26
...
#define BB_WINDOWMINIMIZETOTRAY 10618 /* ... */
...
typedef struct StyleItem
{
	...
	char reserved[128 - (10*4 + 2*1 + 1*2)]; /* keep structure size */
...
#define VALID_OUTLINECOLOR  (1<<16)

#define VALID_FROMSPLITTO   (1<<17)  /* Color.SplitTo */
#define VALID_TOSPLITTO     (1<<18)  /* ColorTo.SplitTo */
...
enum {
	...
	SN_WINFOCUS_BUTTONC         , /* StyleItem* */
	SN_WINFOCUS_BUTTONCP        , /* StyleItem* */
	SN_WINUNFOCUS_BUTTONC       , /* StyleItem* */
...
#ifdef __BBCORE__
#define API_EXPORT(t) DLL_EXPORT t API_CALL
#else
#define API_EXPORT(t) DLL_IMPORT t API_CALL
#endif
...
/* Read Settings */
API_EXPORT(bool) ReadBool(LPCTSTR fileName, LPCTSTR szKey, bool defaultBool);
API_EXPORT(int) ReadInt(LPCTSTR fileName, LPCTSTR szKey, int defaultInt);
API_EXPORT(LPCTSTR) ReadString(LPCTSTR fileName, LPCTSTR szKey, LPCTSTR defaultString);
...
API_EXPORT(LPCTSTR) ReadValue(LPCTSTR fileName, LPCTSTR szKey, LPLONG ptr ISNULL);
...
API_EXPORT(bool) DeleteSetting(LPCTSTR fileName, LPCTSTR szKey); /* wildcards supported for keyword */
API_EXPORT(bool) RenameSetting(LPCTSTR fileName, LPCTSTR old_keyword, LPCTSTR new_keyword);
...
/* ------------------------------------ */
/* File functions */
API_EXPORT(bool) FileExists(LPCTSTR fileName);
API_EXPORT(FILE*) FileOpen(LPCTSTR fileName);
API_EXPORT(bool) FileClose(FILE *filePointer);
API_EXPORT(bool) FileRead(FILE *filePointer, LPTSTR buffer);
API_EXPORT(bool) ReadNextCommand(FILE *filePointer, LPTSTR buffer, DWORD maxLength);
...
/* ------------------------------------ */
/* Window utilities */
API_EXPORT(size_t) GetAppByWindow(HWND Window, TCHAR*, size_t);
...
// only this function was defined as STDCALL type. Why?
API_EXPORT(LPTSTR) GetBlackboxPath(LPTSTR path, size_t maxLength); /* with trailing backslash */

API_EXPORT(LPCTSTR) bbrcPath(LPCTSTR bbrcFileName ISNULL);
API_EXPORT(LPCTSTR) extensionsrcPath(LPCTSTR extensionsrcFileName ISNULL);
API_EXPORT(LPCTSTR) menuPath(LPCTSTR menurcFileName ISNULL);
API_EXPORT(LPCTSTR) plugrcPath(LPCTSTR pluginrcFileName ISNULL);
API_EXPORT(LPCTSTR) stylePath(LPCTSTR styleFileName ISNULL);

/* ------------------------------------ */
/* Get the main window and other info */
API_EXPORT(HWND) GetBBWnd();
API_EXPORT(LPCTSTR) GetBBVersion(); /* for instance: "bbLean 1.13" */
API_EXPORT(LPCTSTR) GetOSInfo();
API_EXPORT(bool) GetUnderExplorer();

/* ------------------------------------ */
/* String tokenizing */
API_EXPORT(LPTSTR) Tokenize(LPCTSTR sourceString, LPTSTR targetString, LPCTSTR delimiter);
API_EXPORT(int) BBTokenize (LPCTSTR sourceString, LPTSTR* targetStrings, DWORD numTokensToParse, LPTSTR remainingString,size_t remainingString_size);

/* ------------------------------------ */
/* Shell execute a command */
API_EXPORT(BOOL) BBExecute(HWND Owner, LPCTSTR szOperation, LPCTSTR szCommand, LPCTSTR szArgs, LPCTSTR szDirectory, int nShowCmd, bool noErrorMsgs);

/* ------------------------------------ */
/* Logging and error messages */
/* API_EXPORT(void) Log(LPCTSTR format, ...); */
...
API_EXPORT(int) BBMessageBox(int flg, const TCHAR *fmt, ...);

/* debugging */
API_EXPORT(void) dbg_printf(const TCHAR *fmt, ...);
...
API_EXPORT(LPTSTR) ReplaceShellFolders(LPTSTR path, size_t path_size);
...
API_EXPORT(void) MakeGradientEx(HDC hdc, RECT rect, int gradientType, COLORREF colourFrom, COLORREF colourTo, COLORREF colourFromSplitTo, COLORREF colourToSplitTo, bool interlaced, int bevelStyle, int bevelPosition, int bevelWidth, COLORREF borderColour, int borderWidth);
...
API_EXPORT(Menu*) MakeNamedMenu(LPCTSTR HeaderText, LPCTSTR Id, bool fshow);
...
API_EXPORT(MenuItem*) MakeMenuNOP(Menu *PluginMenu, LPCTSTR Title ISNULL);

/* inserts an Separator */
...
API_EXPORT(MenuItem*) MakeMenuItemString(Menu *PluginMenu, LPCTSTR Title, LPCTSTR Cmd, LPCTSTR init_string ISNULL);
...
API_EXPORT(MenuItem*) MakePathMenu(Menu *ParentMenu, LPCTSTR Title, LPCTSTR path, LPCTSTR Cmd ISNULL);
...
API_EXPORT(bool) MenuExists(LPCTSTR IDString_start);
...
/* ------------------------------------ */
/* Desktop Information: */
typedef struct string_node { struct string_node *next; TCHAR str[1]; } string_node;

typedef struct DesktopInfo
{
	TCHAR name[32];  /* name of the desktop */
...
#ifndef __BBCORE__
DLL_EXPORT int API_CALL beginPlugin(HINSTANCE);
DLL_EXPORT int API_CALL beginSlitPlugin(HINSTANCE, HWND hSlit);
DLL_EXPORT int API_CALL beginPluginEx(HINSTANCE, HWND hSlit);
DLL_EXPORT void API_CALL endPlugin(HINSTANCE);
DLL_EXPORT LPCTSTR API_CALL pluginInfo(int index);
#endif
