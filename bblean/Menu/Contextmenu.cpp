...
{
public:
	friend Menu *GetContextMenu(const _ITEMIDLIST * pidl);
	//friend class ContextMenu;

	ShellContext(BOOL *, const _ITEMIDLIST *);
	int ShellMenu(void);
...
ContextMenu::ContextMenu (LPCTSTR title, class ShellContext* w, HMENU hm, int m)
	: Menu (title)
{
...
ContextItem::ContextItem(Menu *m, LPTSTR pszTitle, int id, DWORD_PTR data, UINT type)
...
Menu *GetContextMenu(const _ITEMIDLIST * pidl)
{
	BOOL r;
	...
	TCHAR buffer[MAX_PATH]; sh_getdisplayname(pidl, buffer, MAX_PATH);
