...
//#define _strcat(dest, src) _tcsncat(dest, src, sizeof(dest) - sizeof(TCHAR) * (_tcslen(dest) - 1))
//#define _sprintf(str, format, ...) _sntprintf(str, sizeof(str) - sizeof(TCHAR), format, __VA_ARGS__)

//#define CHECKFOCUS

const TCHAR MenuClassName[] = _T("BBMenu");
int g_menu_count;
...
TCHAR g_szSearchString[32] = _T("");
//===========================================================================
// Menu constructor

Menu::Menu(const TCHAR *pszTitle)
{
	ZeroMemory(&this->m_refc, sizeof *this - ((BYTE*)&this->m_refc - (BYTE*)this));
...
{
	//dbg_printf(_T("decref %d %s"), m_refc-1, m_pMenuItems->m_pszTitle);
...
{
	//dbg_printf(_T("incref %d %s"), m_refc, m_pMenuItems->m_pszTitle);
	return ++m_refc;
}
...
Menu *Menu::FindNamedMenu(LPCTSTR IDString)
{
	MenuList *ml;
	dolist (ml, g_MenuStructList)
		if (ml->m->m_IDString && 0==_tcscmp(ml->m->m_IDString, IDString))
		{
			return ml->m;
		}
	return NULL;
...
void Menu::get_vscroller_rect(RECT* rw)
{
	...
	if (DT_LEFT == FolderItem::m_nBulletPosition)
...
int Menu::calc_topindex (bool adjust)
{
	...
	{
		s = (int)imax(d, m_pagesize);
		n = (int)imin(d, (s * m_scrollpos + k/2) / k);
		m_scrollpos = (int)imin(m_scrollpos, d * k / s);
...
void Menu::set_vscroller(int ymouse)
{
	int y0, s, k, i;
	k = get_y_range(&y0, &s);
	i = (int)iminmax(ymouse - s/2 - y0, 0, k);
...
{
	new_top = (int)imax(0, imin(new_top, m_itemcount - m_pagesize));
	if (m_hBitMap)
	{
...
// list, so that it is set to the top of the z-order with _T("Menu_All_BringOnTop()")
...
void Menu::Paint()
{
...
#ifdef CHECKFOCUS
	bool focus = GetAncestor(GetFocus(), GA_ROOTOWNER) == m_hwnd;
	...
	EndPaint(m_hwnd, &ps);
	//dbg_printf(_T("ticks %d"), GetTickCount() - x);
}
...
void Menu::Show(int xpos, int ypos, int flags)
{
	...
	if (p) // a submenu
	{
		int overlap = mStyle.MenuFrame.borderWidth;
	...
	m_xpos = (int)iminmax(xpos, screen_rect.left, screen_rect.right - m_width);
	m_ypos = (int)iminmax(ypos, screen_rect.top, screen_rect.bottom - (IsPinned() ? MenuInfo.nTitleHeight : m_height));
...
void Menu::MenuTimer(WPARAM nTimer)
{
...
void Menu::Validate()
{
	...
	TCHAR current_optional_command[2*MAX_PATH];
	...
	while (NULL != (Item = Item->next))
	{
		...
		{
			const TCHAR *cmd = Item->m_pszCommand;
			if (0 == current_optional_command[0])
				init_check_optional_command(cmd, current_optional_command, 2*MAX_PATH);
			Item->m_isChecked = 0 == _tcsicmp(current_optional_command, cmd);
		}
	...
	m_width = (int)imin(
		...
		(int)imax(w1 + 2 * (MenuInfo.nTitleIndent + border),
			 w2 + 2 * (MenuInfo.nItemIndent + margin)
			 ));
...
void track_mouse_event(HWND hwnd, bool indrag)
{
	...
	else
	{
		...
		SetTimer(hwnd,
				 ...
				 (int)iminmax(Settings_menuPopupDelay-10, 20, 100), NULL);
	}
}
				...
				if (WM_LBUTTONDOWN == uMsg)
				{
					...
					if (m_pChild)
					{
						if (m_pChild->m_pParentItem->within(pt.y))
						{
							...
							bool bullet_left = DT_LEFT == FolderItem::m_nBulletPosition;
							if (at_right == bullet_left) return;
						}
					}
				mouse_leave();
				return;
...
void Menu::start_drag(const TCHAR *arg, const _ITEMIDLIST *c_pidl)
{
...
//===========================================================================
MenuItem * Menu::match_next_shortcut(const TCHAR *d)
{
	MenuItem *Item; const TCHAR *s, *t, *e; unsigned n = 2; size_t dl = _tcslen(d);
	for (Item = m_pActiveItem; ; Item = m_pMenuItems)
	{
		if (Item) do
		{
			if (!m_bSearch) Item = Item->next;
			if (Item == NULL) break;
			s = _tcschr(t = Item->m_pszTitle, _T('&'));
			e = s && s[1] != s[0] ? &s[1] : &t[0];
			if (_tcslen(e) >= dl
				&& 2 == CompareString(
					LOCALE_USER_DEFAULT,
...
//--------------------------------------
bool Menu::Handle_Key(UINT msg, WPARAM wParam)
{
	int n, a, c, d, e, i;
	MenuItem *Item; bool ctrl, shift; const int c_steps = 1;
	...
	if (MENU_ID_INT & m_MenuID)
	{
		...
		if (MENU_ID_STRING & m_MenuID)
		{
			...
			if (
				(WM_CHAR == msg) &&
				(
					!Settings_menuKeyBindVI |
					(g_szSearchString[0] == _T('/')) |
					(wParam == _T('/'))
					)
				){
				TCHAR chars[80]; MSG msg; i = 0;
				chars[i++] = (TCHAR)wParam;
				while(PeekMessage(&msg, m_hwnd, WM_CHAR, WM_CHAR, PM_REMOVE) && (unsigned)i < sizeof(TCHAR) * (sizeof(chars) - 1))
					chars[i++] = (TCHAR)msg.wParam;
				chars[i] = 0;

				//dbg_printf(_T("chars <%s>"), chars);
				if (Settings_menuKeyBindVI){
					if (wParam == _T('/')){
						g_szSearchString[0] = _T('\0');
						m_bSearch = true;
					}

					if (m_bSearch){
						if (wParam == VK_BACK){
							g_szSearchString[_tcslen(g_szSearchString) - 1] = _T('\0');
						}
						else{
							_tcsncat_s(g_szSearchString,32,chars,_TRUNCATE);
						}
					}
					else if(wParam != _T('n')){
						return false;
					...
					if (TCHAR *p = _tcschr(g_szSearchString, _T('/'))){
						_tcsncpy_s(chars, 80, p+1, _TRUNCATE);
						TCHAR buf[32];
						_sntprintf_s(buf,32,_TRUNCATE,_T("%s|"), g_szSearchString);
						SetTitle(m_bSearch ? buf : g_szSearchString);
					}
					else{
						m_bSearch = false;
						ResetTitle();
				...
				// Shortcuts
				Item = !_tcscmp(g_szSearchString, _T("/")) ? NULL : match_next_shortcut(chars);
				if (Item)
				{
					hilite(Item);
					if (!Settings_menuKeyBindVI){
				...
				if (WM_KEYDOWN == msg)
				{
					...
					d = m_pagesize - 1;
					n = 0;
					ctrl = ((0x8000 & GetKeyState(VK_CONTROL)) == 0x8000);
					shift = ((0x8000 & GetKeyState(VK_SHIFT)) == 0x8000);
				one_more:
					i = 1;
					switch(wParam)
					{
					...
					case _T('J'): if (!Settings_menuKeyBindVI | m_bSearch) break;
						goto case_vk_down;
					case _T('K'): if (!Settings_menuKeyBindVI | m_bSearch) break;
						goto case_vk_up;
					case _T('H'): if (!Settings_menuKeyBindVI | m_bSearch) break;
						goto case_vk_left;
					case _T('L'): if (!Settings_menuKeyBindVI | m_bSearch) break;
						goto case_vk_right;
					case _T('O'): if (!Settings_menuKeyBindVI | m_bSearch) break;
						goto case_vk_return;
					case _T('G'): if (!Settings_menuKeyBindVI | m_bSearch) break;
						if (m_LastKey == _T('G')){
							if (!shift & !ctrl){
								goto case_vk_home;
							}
						...
						break;
					case _T('P'): if (!Settings_menuKeyBindVI | m_bSearch) break;
						if (m_pActiveItem)
						{
					...
					case VK_DOWN:
						if (ctrl)
						{
							i = (int)imin(c_steps, e - d - m_topindex);
							if (i >= 0)
								scroll_menu(i);
							else
								break;
					...
					case VK_UP:
						...
						if (a < 0) a = e;
							...
							if (0 == a) a = (d < e) ? 0 : e;
							else a = (int)imax(a-i, 0);
						goto set;

						//--------------------------------------
					case VK_NEXT:
						if (a < 0) a = m_topindex;
						c = (int)imin (e - d - m_topindex, d);
						if (c <= 0) a = e;
						else a += c;
						scroll_menu(c);
						wParam = VK_DOWN;
					...
					case VK_SPACE:
						...
						if (m_pActiveItem)
						{
							if (shift | ctrl)
							{
								m_pActiveItem->Invoke(INVOKE_DBL|INVOKE_LEFT);
							}
							else
							{
								m_pActiveItem->Invoke(INVOKE_LEFT);
...
LRESULT CALLBACK Menu::WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	//dbg_printf(_T("hwnd %x  msg %x  wp %08x  lp %08x"), hwnd, uMsg, wParam, lParam);
...
#ifdef CHECKFOCUS
		dbg_printf(_T("WM_KILLFOCUS %s"), p->m_pMenuItems->m_pszTitle);
...
#ifdef CHECKFOCUS
		dbg_printf(_T("WM_SETFOCUS %s"), p->m_pMenuItems->m_pszTitle);
		InvalidateRect(hwnd, NULL, FALSE);
#endif
		break;
	...
	case WM_MOUSEWHEEL:
		...
		//====================
	case WM_MOUSELEAVE:
		//dbg_printf(_T("mouseleave %x"), hwnd);
		if (p->m_bMouseOver)
		{
	...
	case WM_TIMER:
		...
		//dbg_printf(_T("Timer %x %d"), hwnd, wParam);
		p->MenuTimer(wParam);
		break;
...
void SetFolderItemCommand(MenuItem *Item, const TCHAR *buf)
{
...
void MenuItemInt_SetOffValue(class MenuItem *Item, int n, const TCHAR *p)
{
	Item = get_real_item(Item);
	if (Item)
	{
		((IntegerItem*)Item)->offvalue = n;
		((IntegerItem*)Item)->offstring = p ? p : NLS0(_T("off"));
	}
}
...
void Menu::redraw_structure(void)
{
	...
	//if (NULL == m_pMenuItems->next) MakeMenuNOP(this, _T("---"));
...
//===========================================================================
void Menu::SetTitle(LPTSTR pszTitle){
	free_str(&m_pMenuItems->m_pszTitle);
	m_pMenuItems->m_pszTitle = new_str(pszTitle);
}

void Menu::ResetTitle(void){
	if (_tcsicmp(m_pMenuItems->m_pszTitle, m_pMenuItems->m_pszTitle_old))
		SetTitle(m_pMenuItems->m_pszTitle_old);
}

//===========================================================================
// The menu is invoked, either with the mouse or with a keystroke

void Menu_ShowFirst(Menu *p, bool from_kbd)
{
	p->m_bIconized = false;
	p->redraw_structure();
	if (NULL == p->m_hwnd)
...
API_EXPORT(Menu*) MakeNamedMenu(LPCTSTR HeaderText, LPCTSTR IDString, bool popup)
{
	Menu *pMenu = NULL;
	...
	//dbg_printf(_T("MakeNamedMenu %x %s <%s>"), pMenu, HeaderText, IDString);
	return pMenu;
}
...
API_EXPORT(Menu*) MakeMenu(LPCTSTR HeaderText)
{
...
{
	//dbg_printf(_T("ShowMenu %x %s"), PluginMenu, PluginMenu->m_pMenuItems->m_pszTitle);
...
API_EXPORT(void) DelMenu(Menu *PluginMenu) { } // not required anymore.

//===========================================================================
// API: MakeSubmenu
//===========================================================================

API_EXPORT(MenuItem*) MakeSubmenu(Menu *ParentMenu, Menu *ChildMenu, LPCTSTR Title)
{
	//dbg_printf(_T("MakeSubmenu %x %s - %x %s"), ParentMenu, ParentMenu->m_pMenuItems->m_pszTitle, ChildMenu, Title);
	return ParentMenu->AddMenuItem(new FolderItem(ChildMenu, NLS1(Title), NULL));
}
...
MenuItem* MakeSubmenu(Menu *ParentMenu, Menu *ChildMenu, LPCTSTR Title, LPCTSTR Icon)
{
	//dbg_printf(_T("MakeSubmenu %x %s - %x %s"), ParentMenu, ParentMenu->m_pMenuItems->m_pszTitle, ChildMenu, Title);
	return ParentMenu->AddMenuItem(new FolderItem(ChildMenu, NLS1(Title), Icon));
}
...
API_EXPORT(MenuItem*) MakeMenuItem(Menu *PluginMenu, LPCTSTR Title, LPCTSTR Cmd, bool ShowIndicator)
{
	//dbg_printf(_T("MakeMenuItem %x %s"), PluginMenu, Title);
	return PluginMenu->AddMenuItem(new CommandItem(Cmd, NLS1(Title), ShowIndicator, NULL));
}
...
MenuItem* MakeMenuItem(Menu *PluginMenu, LPCTSTR Title, LPCTSTR Cmd, bool ShowIndicator, LPCTSTR Icon)
{
	//dbg_printf(_T("MakeMenuItem %x %s"), PluginMenu, Title);
	return PluginMenu->AddMenuItem(new CommandItem(Cmd, NLS1(Title), ShowIndicator, Icon));
}

//===========================================================================
// Create the one item submenu for MakeMenuItemInt and MakeMenuItemString

MenuItem * make_helper_menu(Menu *PluginMenu, LPCTSTR Title, int menuID, MenuItem *Item)
{
	Menu *sub;
	if (PluginMenu->m_IDString)
	{
		TCHAR buff[256];
		_sntprintf_s (buff, 256, _TRUNCATE, _T("%s.%s"), PluginMenu->m_IDString, Title);
...
API_EXPORT(MenuItem*) MakeMenuItemInt(Menu *PluginMenu, LPCTSTR Title, LPCTSTR Cmd, int val, int minval, int maxval)
{
...
API_EXPORT(MenuItem*) MakeMenuItemString(Menu *PluginMenu, LPCTSTR Title, LPCTSTR Cmd, LPCTSTR init_string)
{
...
API_EXPORT(MenuItem*) MakeMenuNOP(Menu *PluginMenu, LPCTSTR Title)
{
	MenuItem *pItem = PluginMenu->AddMenuItem(new MenuItem(Title ? NLS1(Title) : _T("")));
...
API_EXPORT(MenuItem*) MakeMenuSEP(Menu *PluginMenu)
{
	return PluginMenu->AddMenuItem(new SeparatorItem());
}
...
MenuItem* MakeMenuVOL(Menu *PluginMenu, LPCTSTR Title, LPCTSTR DllName)
{
	return PluginMenu->AddMenuItem(new VolumeItem(Title, DllName));
}
...
API_EXPORT(MenuItem*) MakePathMenu(Menu *ParentMenu, LPCTSTR Title, LPCTSTR path, LPCTSTR Cmd)
{
	return ParentMenu->AddMenuItem(new SpecialFolderItem(NLS1(Title), path, NULL, Cmd, NULL));
}
...
MenuItem* MakePathMenu(Menu *ParentMenu, LPCTSTR Title, LPCTSTR path, LPCTSTR Cmd, LPCTSTR Icon)
{
...
API_EXPORT(bool) MenuExists(const TCHAR* IDString_part)
{
	size_t l = _tcslen(IDString_part);
	MenuList *ml;
	dolist (ml, Menu::g_MenuStructList)
		if (ml->m->m_IDString && 0 == _tcsncmp(ml->m->m_IDString, IDString_part, l))
		{
			return true;
		}
	return false;
...
API_EXPORT(void) DisableLastItem(Menu *PluginMenu)
{
	if (PluginMenu)
	{
		...
		I1 = (MenuItem*)((BYTE*)PluginMenu->m_ppAddItem - ((BYTE*)&I1->next - (BYTE*)I1));
