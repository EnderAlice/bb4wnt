...
int __cdecl SpecialFolder_Compare(MenuItem** m1, MenuItem** m2)
{
	int i, f;
	ptrdiff_t x, y, z, ip;
	const TCHAR *a1, *b1, *a2, *b2;
	...
	if (0!=(i=_tcsicmp(a2,b2))) return i;
	x=a2-a1;
	y=b2-b1;
	if (0!=(i=_tcsnicmp(a1,b1,x<y?x:y))) return i;
	if (0!=(ip=(ptrdiff_t)x-(ptrdiff_t)y)) return SIGN(ip);
	return SIGN((BYTE*)*m2 - (BYTE*)*m1);

}

MenuItem * MenuItem::Sort(int(__cdecl *cmp_fn)(MenuItem**,MenuItem**))
{
	...
	qsort(a, n, sizeof *a, (int(__cdecl *)(const void*,const void*))cmp_fn);
