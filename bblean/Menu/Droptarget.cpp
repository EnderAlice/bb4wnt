...
{
public:
	CDropTarget(HWND hwnd, const _ITEMIDLIST * pidl)
	{
		m_dwRef = 1;
		m_pDropTarget = NULL;
		m_hwnd = hwnd;
...
private:
	...
	friend class CDropTarget *init_drop_targ(HWND hwnd, const _ITEMIDLIST * pidl);
...
{
#if 0
	_ITEMIDLIST * pidl = (_ITEMIDLIST *)SendMessage(m_hwnd, BB_DRAGOVER, grfKeyState, (LPARAM)&pt);
	// -----------------------------------------------------------
...
//-----------------------------------------------------------------------------
class CDropTarget *init_drop_targ(HWND hwnd, const _ITEMIDLIST * pidl)
{
