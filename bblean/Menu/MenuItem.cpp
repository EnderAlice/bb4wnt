...
MenuItem::MenuItem(const TCHAR* pszTitle)
{
	next        = NULL;
...
//====================
void MenuItem::ItemTimer(WPARAM nTimer)
{
...
void MenuItem::Mouse(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	int i = 0;
	switch(uMsg)
	{
...
const TCHAR* MenuItem::GetDisplayString(void)
{
...
{
	const TCHAR *title = GetDisplayString();
	RECT r = { 0, 0, 0, 0 };
...
void MenuItem::Paint(HDC hDC)
{
	...
	StyleItem *pSI = (StyleItem*)c_alloc(sizeof(StyleItem));
	memcpy(pSI, &mStyle.MenuFrame, sizeof(StyleItem));
	bool lit = false;
	...
	if (m_bActive && 0 == (m_isNOP & (MI_NOP_TEXT | MI_NOP_SEP)) && (0 == (m_isNOP & MI_NOP_DISABLED) || m_pSubMenu))
	{
		// draw hilite bar
		GetItemRect(&rect);
		memcpy(pSI, &mStyle.MenuHilite, sizeof(StyleItem));
		MakeStyleGradient(hDC, &rect, pSI, pSI->bordered);
		lit = true;
	}
	else if (m_isNOP & MI_NOP_DISABLED)
		pSI->TextColor = mStyle.MenuFrame.disabledColor;
	else if (m_pSI && !m_bActive)
		memcpy(pSI, m_pSI, sizeof(StyleItem));
	...
	if (m_isChecked) // draw check-mark
	{
		...
		if (lit != pr) memcpy(pSI, &mStyle.MenuFrame, sizeof(StyleItem));
	...
	if(m_phIcon){
		...
		DrawIconEx(hDC, rect.left, rect.top, *m_phIcon, nIconSize, nIconSize, 0, 0, DI_NORMAL);
	}
}

//===========================================================================

HICON* MenuItem::GetIcon(const TCHAR *pszIcon){
	if(pszIcon && *pszIcon)
	{
		int nIconSize = Settings_menuIconSize;
		if (nIconSize)
		{
			TCHAR *pszPath = NULL; // menu icon path(ico, exe, dll, icl ...)
			TCHAR *pszTemp = NULL; // temporary
			int nIndex = 0;       // default icon index is 0
			size_t pszPath_size;

			pszPath_size = _tcslen(pszIcon)+1;
			pszTemp = (TCHAR*)m_alloc(sizeof(TCHAR)*pszPath_size);
			_tcsncpy_s(pszTemp, pszPath_size, pszIcon, _TRUNCATE);

			// get icon index
			// separate path and icon with _T(",")
			if (TCHAR *p = _tcsrchr(pszTemp, _T(',')))
			{
				*p = _T('\0');
				nIndex = _tstoi(p+1);
			}
			...
			pszPath_size = sizeof(TCHAR)*(_tcslen(pszTemp)+1);
			pszPath = (TCHAR*)m_alloc(pszPath_size);
			unquote(pszPath, pszPath_size, pszTemp);

			m_free(pszTemp);

			// TODO: アイコンが複数のサイズを持つ場合に対応できるよう、LoadImageを使うように変更
			// get icon handle
			HICON *phIcon = (HICON*)m_alloc(sizeof(HICON));
			if (nIconSize <= 16){ // small icon
				ExtractIconEx(pszPath, nIndex, NULL, phIcon, 1);
			}
			else{ // large icon
				ExtractIconEx(pszPath, nIndex, phIcon, NULL, 1);
			}
...
void MenuItem::FreeIcon(HICON *phIcon){
	if (phIcon){
		DestroyIcon(*phIcon);
		m_free(phIcon);
	}
