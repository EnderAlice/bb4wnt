...
void vwm_update_winlist(void)
{
	...
	dolist (wl, vwm_WL)
	{
		...
		dbg_printf(_T("hwnd:%x desk:%d moved:%d hidden:%d iconic:%d <%s>"),
...
void fix_iconized_window(winlist *wl)
{
...
void check_owned_windows(HWND hwnd)
{
// 	DWORD dwThreadId = GetWindowThreadProcessId(hwnd, NULL);
	hwnd = GetAncestor(hwnd, GA_ROOTOWNER);
	winlist *wl;
	dolist (wl, vwm_WL)
		wl->check = hwnd == GetAncestor(wl->hwnd, GA_ROOTOWNER);
// 			dwThreadId == GetWindowThreadProcessId(wl->hwnd, NULL)
...
bool set_location_helper(HWND hwnd, struct taskinfo *t, UINT flags)
{
	winlist *wl = (winlist*)assoc(vwm_WL, hwnd);
